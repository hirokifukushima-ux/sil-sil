...
-- users テーブルに外部キー制約を追加（カラムが既に存在する場合）
DO $$
BEGIN
  -- parent_id の外部キー制約
  IF NOT EXISTS (
  ...
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.table_constraints
    WHERE constraint_name = 'users_master_id_fkey'
  ) THEN
    ALTER TABLE users ADD CONSTRAINT users_master_id_fkey
