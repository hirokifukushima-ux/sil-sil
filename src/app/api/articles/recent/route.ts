...
export async function GET(request: NextRequest) {
  try {
    ...
    const childId = childIdParam || undefined;
    const includeArchived = searchParams.get('includeArchived') === 'true';

    // 親アカウントIDを決定（子の場合は親のIDを使用）
    const parentId = session.userType === 'parent' ? session.userId : session.parentId;
    
    console.log(`📊 最近の記事を取得中... (親: ${parentId}, childId: ${childId || 'N/A'}, childAge: ${childAge || 'all'}, limit: ${limit}, includeArchived: ${includeArchived})`);
    ...
    const [articles, stats] = await Promise.all([
      db.getArticles({
        ...
        childAge: childAge, // 子どもの年齢でフィルタリング（childIdがない場合・後方互換性）
